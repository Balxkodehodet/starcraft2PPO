...
class SC2ContinuousCNNEnv(gym.Env):
    ...
    def step(self, action):
        ...
        try:
            ...
            # Safety: ensure at least one valid action (no all-zero masks)
            if not np.any(action_mask):
                print("⚠️ No valid actions reported by SC2 — using fallback 'no_op'.")
                action_mask[:] = 0.0
                # find "no_op" index safely
                no_op_id = sc2_actions.FUNCTIONS.no_op.id
                if no_op_id in self.valid_funcs:
                    action_mask[self.valid_funcs.index(no_op_id)] = 1.0
                else:
                    action_mask[0] = 1.0  # fallback fallback
        except Exception as e:
            print(f"⚠️ Exception building action mask: {e}")
            action_mask = np.zeros(len(self.valid_funcs), dtype=np.float32)
            action_mask[0] = 1.0  # emergency fallback
        ...
        if not np.any(action_mask):
            print("Warning: all actions masked, fallback to no_op")
            action_mask[0] = 1.0
