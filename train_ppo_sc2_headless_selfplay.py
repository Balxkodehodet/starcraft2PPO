...
class SC2ContinuousCNNEnv(gym.Env):
    ...
    def set_opponent_policy(self, policy):
        """
        Set or update the opponent's policy used in self play.
        'policy' is a stable-baseline3 model with a .predict() method
        """
        self.opponent_policy = policy
        print("[SelfPlay] Opponent policy updated")
...
if __name__ == "__main__":
    ...
    if os.path.exists(model_path):
        ...
        model = PPO("CnnPolicy", env, verbose=1, device="cuda", tensorboard_log="./ppo_sc2_logs/")
    ...
    while timesteps_done < TOTAL_TIMESTEPS:
        ...
        for env_idx in range(NUM_ENVS):
            env.envs[env_idx].opponent_policy = frozen_opponent.predict
